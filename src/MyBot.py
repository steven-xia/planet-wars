...
def redistribute(pw):
    ...
    for planet in pw.my_planets():
        ...
        closest_planet = min(pw.enemy_planets() + list(pw.enemy_future_neutrals),
                             key=lambda p: pw.distance(p.planet_id(), planet.planet_id()))
        for other_planet in sorted(redistribute_planets, key=lambda p: pw.distance(closest_planet.planet_id(),
                                                                                   p.planet_id())):
            redistribute_distance = pw.distance(planet.planet_id(), other_planet.planet_id())
            ...
            for enemy_planet in list(enemy_keep_future_planets) + list(enemy_future_planets):
                ...
                to_enemy = pw.distance(planet.planet_id(), enemy_planet.planet_id())
                if pw.distance(planet.planet_id(), other_planet.planet_id()) > to_enemy or \
                        pw.distance(other_planet.planet_id(), enemy_planet.planet_id()) > to_enemy:
                    break
            else:
